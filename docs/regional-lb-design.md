...
AllowedIPs = <exact full firm-bypass complement generated by the controller>
PersistentKeepalive = 25
```

The controller excludes all configured server `/24`s from every split profile.
On macOS, `awg-en1-route` also resolves the endpoint hostname and pins each A
record to the en1 gateway as a second safeguard.
...
# os_type: macos | windows | ios | android
# routing: full | split
```
Output files are in `/tmp/output/`:
- `<device_name>.conf` — AWG config to import on the device. Mobile files are
  capped at 32 KiB; desktop files retain the exact full bypass dataset.
- `<device_name>-qr.conf` and `<device_name>-qr.png` — iOS/Android only; priority
  WeChat/Taobao/Douyin profile capped at 2000 bytes.
...
The main mobile and QR profiles share one key and tunnel address. They are
alternative imports for one device and must not be enabled simultaneously.

QR code for mobile is generated automatically when `qrencode` is installed. To
render it in a terminal manually:
```bash
qrencode -l M -t ansiutf8 < /tmp/output/<device_name>-qr.conf
```
...
### VPN-default firm bypass

All platforms use the same VPN-default policy with platform-specific detail:

| Destination | Route |
|-------------|-------|
| Curated major Chinese internet-firm IPv4 ranges | Direct |
| Local/special IPv4 and active AWG endpoint networks | Direct |
| Every other public IPv4 destination | VPN |
| IPv6 in split mode | Not installed |

`vpn-update-china-firm-bypass.timer` resolves the curated domains through AliDNS
and DNSPod each day and combines the observed `/24`s with static safety nets.
`provision.py` computes the IPv4 complement for `AllowedIPs`. Desktop profiles
retain the exact full dataset. Mobile file profiles broaden narrow observations
only as needed to stay at or below 32 KiB, never beyond `/16`. Mobile QR profiles
use a separate WeChat/Taobao/Douyin dataset, compact without spaces, and broaden
only as needed to keep the complete configuration at or below 2000 bytes.

The source inputs are:

```text
/etc/vpn-controller/china-firm-bypass-domains.txt
/etc/vpn-controller/china-firm-bypass-static-cidrs.txt
/etc/vpn-controller/china-qr-bypass-domains.txt
/etc/vpn-controller/china-qr-bypass-static-cidrs.txt
```

The generated last-known-good set is:

```text
/etc/vpn-controller/china-firm-bypass-cidrs.txt
/etc/vpn-controller/china-qr-bypass-cidrs.txt
```
...
only; production iOS/Android profiles should use either the 32 KiB file-import
profile or the 2000-byte priority QR profile, both with `MTU = 1180`, IPv4 DNS
only, and `PersistentKeepalive = 10`.

### Current server IP coverage

| Server | IP | In AllowedIPs? | Action required |
|--------|----|----------------|-----------------|
| av1 Alibaba Tokyo | 47.245.61.67 | No — endpoint `/24` is carved out | None |
| minipc | 125.229.161.122 | No — endpoint `/24` is carved out | None |

All generated split configs exclude every configured server `/24`. macOS also
runs the route-pinner, while mobile relies on this exclusion alone.

### Updating the bypass set

Edit the domain or static-CIDR source, then refresh the generated last-known-good
set. Provisioning reads that generated file; it does not fetch DNS inline.

```bash
sudo systemctl start vpn-update-china-firm-bypass.service
cat /var/lib/vpn-controller/china-firm-bypass-last-update.txt
```

### Adding a new server — AllowedIPs checklist

1. Add the server and its public IP to `controller.yaml`.
2. Reprovision clients that may use it. The generator automatically excludes
   every configured server `/24` before calculating `AllowedIPs`.
3. For mobile, confirm the main file is at most 32768 bytes and the QR file is at
   most 2000 bytes. Desktop files intentionally have no mobile size cap. The API
   and `client/reprovision.sh` enforce the applicable limits.
4. macOS keeps `awg-en1-route` enabled as a second endpoint-routing safeguard.
...
- [x] `controller/update-china-firm-bypass.py` + timer — refresh compact direct ranges through mainland DNS
