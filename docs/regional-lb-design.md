...
macOS does not need server IP exclusions baked into AllowedIPs. The route-fix daemon (`fix-hysteria-route.sh`) reads `servers.conf` at runtime and installs `/32` host routes for every server IP via en1, taking precedence over any matching AllowedIPs CIDR. This means even if a server IP falls inside a covered block (e.g. minipc at 125.229.161.122 falls inside `125.224.0.0/12`), the tunnel still works correctly.
...
**macOS** uses the full uncompressed non-China CIDR list stored at `/etc/vpn-controller/split-allowed-ips.txt` on tn1. It is generated once and updated only when the China routing table changes significantly. Server IPs do not need to be excluded because `fix-hysteria-route.sh` adds `/32` host routes via en1 that take precedence over any AllowedIPs match.
...
4. macOS: update `servers.conf` and the route-fix daemon handles the rest automatically.
