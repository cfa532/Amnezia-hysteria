...
| [`regional-lb`](../../tree/regional-lb) | **AmneziaWG only, server-side load balancer.** Hysteria2 is absent. A FastAPI provisioning API assigns clients to the least-loaded server and manages DNS. Health checks via SSH. Use this if you want automated provisioning without the Hysteria2 transport layer. **This branch.** |
